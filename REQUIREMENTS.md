...
**Precio actual — automático para Stock/ETF/Fondo Indexado/Criptomoneda (Commodity NO, ver más abajo)**: una Edge Function de Supabase (`supabase/functions/actualizar-precios-patrimonio`), programada por `pg_cron` cada hora en punto entre las 6:00 y las 22:00 UTC (ver `supabase/migrations/0012_patrimonio_cron_horario.sql`) — rango elegido para cubrir siempre las 8:00-23:00 hora de España durante todo el año sin depender de `cron.timezone`, un parámetro de pg_cron que solo se puede fijar al arrancar el servidor de Postgres y que por tanto no está al alcance del SQL Editor en un Postgres gestionado por Supabase (error `55P02` al intentarlo) — actualiza `precio_actual_unitario` de cada posición activa sin `tae` consultando en vivo el ticker guardado — sin ninguna lista intermedia que mantener a mano: se consulta directamente con el ticker de la posición en el momento de la ejecución. Fuentes: **CoinGecko** (gratis, sin clave) para Criptomoneda — el campo Ticker debe ser el **ID de CoinGecko**, no el símbolo bursátil (p. ej. `bitcoin`, no `BTC`); **Twelve Data** (clave gratuita, 800 peticiones/día) para Stock/ETF/Fondo Indexado — el Ticker es el símbolo de Twelve Data, y el campo Mercado (opcional) se manda como parámetro `exchange` para desambiguar si el símbolo existe en varias bolsas. El precio siempre se pide en el momento con el ticker que ya tiene la posición: dar de alta una posición nueva no requiere ningún paso de configuración aparte, más allá de escribir el ticker en el formato correcto. Fondo Monetario, Cuenta Remunerada, Cuenta de Ahorro y Cuenta Corriente (posiciones "de saldo") no tienen ticker, así que no entran en esta actualización automática — para las 3 primeras, ver la TAE más abajo; Cuenta Corriente siempre es manual.
