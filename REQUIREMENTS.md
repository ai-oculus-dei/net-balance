...
**El campo "Precio actual" se bloquea (solo lectura) cuando el precio se actualiza solo**: para Stock/ETF/Fondo Indexado/Criptomoneda con un Ticker puesto (`esTipoConPrecioAutomatico`, `src/lib/finance/patrimonio.ts`), el formulario no deja editar "Precio actual" a mano — muestra el valor guardado como texto, con una nota de que se actualiza solo. Si se dejara editar, el valor manual solo duraría hasta la siguiente ejecución del cron (como mucho 1 hora), y mientras tanto el cálculo de P&L/rentabilidad (que siempre lee `precio_actual_unitario` tal cual, sin distinguir su origen) reflejaría ese valor manual en vez del de mercado. Bloquear la edición en el formulario es más simple y fiable que intentar que el cálculo "ignore" un valor manual: los dos usan la misma columna, así que evitar que se pueda escribir uno a mano ya garantiza que el cálculo siempre refleja el último precio de Twelve Data/CoinGecko. Commodity queda fuera de este bloqueo (su Ticker no se actualiza solo, ver arriba) y sigue siempre editable a mano.

**Commodity — sin actualización automática (limitación de plan, no de código)**: aunque el símbolo existe en el catálogo de Twelve Data (p. ej. `XAU/EUR` para oro, `XAG/EUR` para plata — aparecen en `symbol_search`, el buscador gratuito), servir su precio es una función de "Commodities market data", que Twelve Data solo incluye a partir de su plan de pago Grow (~29 USD/mes) — la clave gratuita usada por esta app no tiene acceso, y la petición falla en silencio (la posición se queda con el último precio guardado). No hay ninguna fuente gratuita equivalente conectada; las posiciones de tipo Commodity deben darse de alta **sin Ticker** y actualizar el Precio actual a mano.
