...
**Regla especial — "mes" personalizado (periodo de nómina a nómina):** por defecto la app agrupa movimientos por mes de calendario (día 1 al último día). Cada usuario puede redefinir esto marcando la casilla **"Hacer primer día del mes"**, que aparece justo antes del campo Fecha al registrar un ingreso en la subcategoría **Salario** (Finanzas): ese día pasa a ser el inicio de un periodo personal que dura hasta el día antes de la siguiente nómina marcada (que, a su vez, empieza el suyo). La etiqueta de calendario del periodo (p. ej. "Septiembre") es el mes siguiente al mes en que cae la fecha marcada, decidida en el momento de marcar la casilla, sin esperar a la siguiente nómina. Ejemplo: nómina marcada el 26 de agosto → ese periodo se llama "Septiembre"; si la siguiente nómina se marca el 28 de septiembre, "Septiembre" queda fijado en [26 agosto, 27 septiembre] y "Octubre" empieza el 28 de septiembre. Cada usuario tiene su propio calendario, calculado solo a partir de sus propias nóminas marcadas (`usuario_id`) — no se comparte entre los 2 usuarios aunque compartan movimientos. Si para un mes de calendario no hay ninguna nómina marcada que lo defina (datos históricos antes de usar esta función, o una casilla olvidada), ese mes cae al mes de calendario normal, recortado para no solaparse con un periodo real vecino (si el periodo del mes anterior sigue abierto y ya cubre esas fechas, el mes sin marcar queda vacío en vez de duplicar esos movimientos). Esta redefinición de "mes" se aplica en toda la app — Inicio (mes en curso), Movimientos (selector de mes) y Visualizaciones (rango Desde/Hasta) — pero no al cálculo de "meses restantes" de un objetivo de ahorro automático (sección 7), que sigue usando meses de calendario para esa estimación de planificación futura.
