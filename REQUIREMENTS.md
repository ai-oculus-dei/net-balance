...
- Tipos "de saldo" (sin Tickr: Cuenta Corriente, Cuenta de Ahorro, Cuenta Remunerada, Fondo Monetario): mismo Tipo + mismo Nombre. Como no hay Tickr que escribir, la asociación es explícita: si ya existe alguna posición activa de ese mismo Tipo, aparece un desplegable "Cuenta" bajo el Tipo con los nombres de las cuentas existentes de ese tipo más la opción "Nueva" (la que sale seleccionada por defecto). Elegir una cuenta existente rellena y bloquea el campo Nombre con ese nombre, y la nueva aportación (aunque se guarda como una posición/compra distinta, con su propia fecha y — si aplica — su propia TAE) se suma a esa cuenta en la tarjeta agrupada, como si fuera un ingreso más a la misma cuenta. Elegir "Nueva" (o no tener ninguna cuenta de ese tipo todavía) deja el campo Nombre libre y crea una cuenta sin relación con las demás.
