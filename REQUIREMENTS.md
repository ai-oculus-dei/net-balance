...
**Agrupación por activo**: varias compras del mismo activo en fechas distintas (mismo Tickr + mismo Mercado, sin distinguir mayúsculas/espacios) se funden en una sola tarjeta en vez de aparecer como posiciones sueltas — el nombre mostrado es el de la compra más antigua ("primera compra"), el precio de compra mostrado es la media ponderada por cantidad de todas las compras, y el P&L € y P&L % son el agregado de todas ellas (P&L € = suma de cada compra; P&L % = P&L € total / coste total × 100, no una media simple de los porcentajes de cada compra). Pulsar la tarjeta despliega cada compra individual con su propia fecha, cantidad y P&L calculado con su propio precio y cantidad; pulsar una compra desplegada abre su edición. Solo aplica a los tipos "por unidad" (con Tickr): las posiciones "de saldo" nunca se agrupan entre sí. Al dar de alta o editar una posición, si el Tickr+Mercado ya coincide con otra posición activa existente, se muestra un aviso bajo el campo Tickr ("Este activo ya existe en el patrimonio, se heredará el nombre de la primera compra") y el campo Nombre se bloquea con el nombre heredado — evita tener el mismo activo con dos nombres distintos por error.
