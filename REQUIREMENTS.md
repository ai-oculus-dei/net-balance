...
**Commodity — el símbolo "de materia prima" no funciona, pero un ETC/ETF que replique su precio sí**: el símbolo específico de materia prima (p. ej. `XAU/EUR` para oro, `XAG/EUR` para plata) está detrás del plan de pago Grow de Twelve Data (~29 USD/mes) — la clave gratuita usada por esta app no tiene acceso y la petición falla (queda registrado en `error_precio`, ver más abajo). Sin embargo, un ETC/ETF que replique el precio de esa materia prima cotiza como instrumento normal en Twelve Data (no como "Commodity"), así que sí está cubierto por la clave gratuita — p. ej. Xetra-Gold (ticker `4GLD`, mercado `XETR`) para oro, cuya participación representa 1 gramo de oro físico. Se recomienda dar de alta la posición con el ticker de un ETC/ETF así en vez de sin ticker, para no perder la actualización automática.
