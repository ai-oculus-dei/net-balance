...
- [ ] Ejecutar `supabase/migrations/0009_patrimonio.sql` en el SQL Editor de Supabase (nuevas tablas `posiciones_patrimonio`/`patrimonio_historico`, RLS y función `generar_snapshot_patrimonio` — nueva página "Patrimonio", sección 15).
...
- [ ] El backfill del histórico de Patrimonio (`generar_snapshot_patrimonio`) rellena los días saltados usando el precio actual VIGENTE en el momento de ejecutarse, no el valor real que tuviera la posición ese día pasado — no se guarda un histórico de precios de mercado independiente, así que no hay forma de saber el valor exacto de un día no visitado. Si se añade una posición con una fecha de compra muy antigua, el histórico retroactivo saldrá como una línea plana al precio actual hasta que se empiece a actualizar el precio día a día.
- [ ] El snapshot diario de Patrimonio no se reescribe una vez generado ese día: si editas el "Precio Actual" de una posición después de que la app ya haya generado el snapshot de hoy, el punto de "hoy" en el histórico se queda con el precio anterior hasta el snapshot de mañana (aunque el total en vivo de la página ya refleje el nuevo precio).
